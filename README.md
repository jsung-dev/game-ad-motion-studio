...
기존 MP4에 배경 없는 텍스트·PNG 그래픽과 프레임 기반 모션을 직접 합성해 선택한 화면 비율의 MP4로 출력하는 독립형 로컬 MVP입니다. 메인 화면은 `/`, 영상 API는 `/api/video-ad/*`에서 제공합니다.
...
1. MP4를 선택하거나 드롭합니다. 서버의 ffprobe가 실제 MP4 여부, 100MB 제한, 30초 제한을 확인합니다.
...
- 로컬 단일 사용자·단일 워커 테스트용입니다. 로그인, 결제, 클라우드 저장, 배포 구성은 없습니다.
- 업로드 API는 최대 100MB 파일을 메모리에 올린 뒤 저장하므로 다중 사용자용 스트리밍 업로드는 후속 작업입니다.
